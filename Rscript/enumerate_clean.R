##Enumerate F-matrices for fully heterochronous ranked tree shapes with n tips
library("ape")
...
#We can then plot our trees
mytree<-tree_from_hetF(F.list[[25]])
plot(mytree)
...
# CRAN packages
cran_packages <- c("ape", "ggplot2", "dplyr", "tidyr", "viridis", "RColorBrewer", "phytools", "cowplot", "gridExtra")
...
## Basic Tree Plot
mytree$tip.label<-c("6","5","4","1")
mytree$node.label<-c(0,2,3)
p1 <- ggtree(mytree, right = TRUE) +
...
p <- ggtree(mytree, right = TRUE) + layout_dendrogram()
tree_data <- p$data
